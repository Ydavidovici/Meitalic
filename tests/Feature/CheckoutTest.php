...
class CheckoutTest extends TestCase
{
    ...
    public function test_checkout_displays_totals_when_cart_has_items()
    {
        ...
        CartItem::create([ 'cart_id' => $cart->id, 'product_id' => $product->id, 'name' => $product->name, 'price' => 100, 'quantity' => 2, 'total' => 200 ]);
        $cart->update(['discount' => 20]);

        $response = $this->actingAs($user)->get(route('checkout'));
        $response->assertOk()->assertViewIs('pages.checkout.index');
        $response->assertViewHas('subtotal', 200);
        $response->assertViewHas('discount', 20);
        $response->assertViewHas('tax', round((200 - 20) * 0.1, 2));
        $response->assertViewHas('total', round(200 - 20 + ((200 - 20) * 0.1), 2));
    }
    ...
    public function test_payment_intent_returns_client_secret_and_amount()
    {
        $user = User::factory()->create();
        $cart = Cart::create(['user_id' => $user->id]);
        $product = Product::factory()->create(['price' => 50]);
        CartItem::create([ 'cart_id' => $cart->id, 'product_id' => $product->id, 'name' => $product->name, 'price' => 50, 'quantity' => 2, 'total' => 100 ]);
        ...
        $this->assertEquals((100 - 10) / 100, $response->json('amount'));
    }
    ...
    public function test_apply_promo_errors_for_invalid_code()
    {
        ...
        CartItem::create([ 'cart_id' => $cart->id, 'product_id' => $product->id, 'name' => $product->name, 'price' => 30, 'quantity' => 1, 'total' => 30 ]);
    ...
    public function test_apply_promo_success()
    {
        ...
        CartItem::create([ 'cart_id' => $cart->id, 'product_id' => $product->id, 'name' => $product->name, 'price' => 40, 'quantity' => 2, 'total' => 80 ]);
        PromoCode::create([ 'code' => 'SAVE5', 'type' => 'fixed', 'discount' => 5, 'max_uses' => null, 'used_count' => 0, 'expires_at' => now()->addDay(), 'active' => true ]);
    ...
    public function test_place_order_fails_when_payment_not_succeeded()
    {
        ...
        CartItem::create([ 'cart_id' => $cart->id, 'product_id' => $product->id, 'name' => $product->name, 'price' => 20, 'quantity' => 1, 'total' => 20 ]);
    ...
    public function test_place_order_successful_flow()
    {
        Config::set('cart.tax_rate', 0);
        $user = User::factory()->create();
        $cart = Cart::create(['user_id' => $user->id, 'promo_code' => 'DISC1', 'discount' => 1]);
        $product = Product::factory()->create(['price' => 10]);
        CartItem::create([ 'cart_id' => $cart->id, 'product_id' => $product->id, 'name' => $product->name, 'price' => 10, 'quantity' => 2, 'total' => 20 ]);
        PromoCode::create([ 'code' => 'DISC1', 'type' => 'fixed', 'discount' => 1, 'max_uses' => null, 'used_count' => 0, 'expires_at' => now()->addDay(), 'active' => true ]);
        ...
        $this->assertDatabaseHas('carts', ['id' => $cart->id, 'discount' => 0, 'promo_code' => null]);
